...
<!-- This stylesheet has been modified by David Sherman on September 29-30, 2008 to attempt to replicate the format of WHOI's finding aid presentation. Related materials and Restrictions were moved under the Administrative Information section, and the Arrangement section was relocated after Administrative Information. Other minor stylistic changes were made. -->
...
<head>
	<style type="text/css">
	h1, h2, h3, h4 {font-family:Book Antiqua}
...
<!-- This part of the template creates a table for the finding aid with	two columns. -->
...
<table style="text-align:left; width:642px; height:119px;" border="0" cellpadding="2" cellspacing="2">
<tbody>
<tr>
<td width="150px"><img alt="WHOI_logo" src="WHOI_logo.jpg"></img></td>
<td style="font-size:13pt; font-family:Book Antiqua"><b>Woods Hole Oceanographic Institution<br/>
<xsl:value-of select="eadheader/filedesc/titlestmt/titleproper"/>, <xsl:value-of select="archdesc/did/unitdate"/><br/>
<br/>
Archival Collection <xsl:value-of select="eadheader/eadid"/><br/>
<xsl:value-of select="archdesc/did/physdesc"/><br/>
</b></td>
</tr>
</tbody>
</table>
...
<xsl:template name="toc">
	<h4 style="font-size:12pt; font-family:Book Antiqua">TABLE OF CONTENTS</h4>

<!-- The Table of Contents template performs a series of tests to determine which elements will be included in the table of contents.  Each if statement tests to see if there is a matching element with content in the finding aid.-->

<!--	<xsl:if test="string(archdesc/did/head)">
	<h4 style="font-size:11pt; font-family:Book Antiqua">
	<b>
...
<!--	<xsl:if test=" or string(archdesc/userestrict/head)
	or string(archdesc/accessrestrict/head)
	<h4 style="font-size:11pt; font-family:Book Antiqua">
	<b>
	<a href="#restrictlink">
...
<!-- Displays the unittitle and unitdates for a c01 if it is a subgrp, subcollection,series or subseries (as evidenced by the value of @level) and numbers them to form a hyperlink to each. Delete this section if you do not wish the c01 titles that are so identified to appear in the table of contents.-->

<!--	<xsl:for-each select="archdesc/dsc/c01[@level='series' or @level='subseries'
			or @level='subgrp' or @level='subcollection']">
	<h4 style="margin-left:40pt; font-size:10pt; font-family:Book Antiqua">
	<b>
	<a>
...
<super><xsl:apply-templates/></super>
</xsl:template>
...
<font style="font-variant: small-caps"><xsl:apply-templates/></font>
</xsl:template>
...
<!--Modified by Yuan Li Jan.15,2008-->
<!-- This template processes all link elements except for title.-->
...
<!--Modified by Yuan Li, 6/19/2008-->
<!--This template converts the id of the target to a name for an HTML anchor.-->
<xsl:template match="c02|c03|c04|c05|c06|c07|c08|c09|c010">
<a name="@id">
...
<div style="margin-left: 25pt; font-size:10.5pt; font-family:Book Antiqua">
<b>
<xsl:apply-templates/>
</b>
</div>
...
<div style="margin-left: 40pt; font-size:10.5pt; font-family:Book Antiqua">
<xsl:apply-templates/>
</div>
</xsl:template>
	
<!-- D.S. - added blockquote - January 26, 2009 -->
...
<!-- Processes the unit date if it is not a child of unittitle but a child of did, the current context.-->
...
<!--This template rule formats the top-level bioghist element and creates a link back to the top of the page after the display of the element.-->
...
<xsl:template match="archdesc/bioghist |
	...
	<xsl:if test="string(child::*)">
		<xsl:apply-templates/>
	<p style="font-size:11pt; font-family:Book Antiqua">
...
<!--This template formats various head elements and makes them targets for links from the Table of Contents.-->
...
<xsl:template match="archdesc/bioghist/p |
	...
	archdesc/odd/note/p">
	<p style="margin-left:15pt; font-size:11pt; font-family:Book Antiqua">
		<xsl:apply-templates/>
	</p>
</xsl:template>
...
<xsl:template match="archdesc/bioghist/bioghist/p |
	...
	<p style="margin-left:15pt; font-size:10pt; font-family:Book Antiqua">
		<xsl:apply-templates/>
	</p>
</xsl:template>

<!-- The next two templates format an arrangement statement embedded in <scopecontent>.
...
<!-- The next three templates format a list within an arrangement statement whether it is directly within <archdesc> or embedded in <scopecontent>.

<xsl:template match="archdesc/scopecontent/arrangement/list/head">
	<div style="margin-left:15pt; font-size:10pt; font-family:Book Antiqua">
	<a name="{generate-id()}">
		<xsl:apply-templates/>
	</a>
	</div>
...
<xsl:template match="archdesc/scopecontent/arrangement/list/item
	...
	<div style="margin-left:25pt; font-size:10pt; font-family:Book Antiqua">
	<a>
...
<!--This template rule formats the top-level related material elements by combining any related or separated materials elements. It begins by testing to see if there related or separated materials elements with content.-->
...
<!--	<xsl:template name="archdesc-relatedmaterial">
	...
	<xsl:text>Related Material</xsl:text>
		...
		<xsl:apply-templates select="archdesc/separatedmaterial/p
			...
			| archdesc/*/separatedmaterial/note/p"/>
	<p style="font-size:10pt; font-family:Book Antiqua">
	<a href="#">Back to TOC</a>
	</p>
	<hr></hr>
	</xsl:if>
...
<!--Apply this template when the controlled terms are entered directly under the controlaccess element.-->
...
<xsl:template mode="direct" match="archdesc/controlaccess">
	...
	<div style="margin-left:15pt; font-size:10.5pt; font-family:BookAntiqua">
		<xsl:apply-templates/>
	</div>
	</xsl:for-each>
	<p style="font-size:10pt; font-family:Book Antiqua">
...
<!--When controlled terms are nested within recursive controlaccess elements, the template for controlaccess/controlaccess is applied.-->
...
<xsl:template match="archdesc/controlaccess/controlaccess">
	<h4 style="font-size:10pt; font-family:Book Antiqua">
		<xsl:apply-templates select="head"/>
	</h4>
	<xsl:for-each select="subject |corpname | famname | persname | genreform | title | geogname | occupation">
	<xsl:sort select="." data-type="text" order="ascending"/>
...
<!--This template rule formats a top-level access and use retriction elements. They are displayed under a common heading. It begins by testing to see if there are any restriction elements with content.-->
...
<!--This template rule formats the head element of top-level elements of administrative information.-->
...
<xsl:template match="custodhist/head
	...
	| archdesc/accruals/head
        | archdesc/userestrict/head
        | archdesc/accessrestrict/head
        | archdesc/relatedmaterial/head
        | archdesc/separatedmaterial/head
...
<!-- The next several templates format the arrangement statements both within and outside of the scopecontent element, DS 9/29/08  -->
...
<xsl:template match="archdesc/arrangement/head"> 
	<h4 style="font-size:11pt; font-family:Book Antiqua">
	<a name="{generate-id()}">
		<xsl:apply-templates/>
	</a>
...
<xsl:template match="archdesc/arrangement/p
	| archdesc/scopecontent/arrangement/note/p">
	<p style="margin-left:15pt; font-size:11pt; font-family:Book Antiqua">
		<xsl:apply-templates/>
	</p>
...
<xsl:template match="archdesc/scopecontent/arrangement/list/head">
	<div style="margin-left:15pt; font-size:10.5pt; font-family:Book Antiqua">
	<a name="{generate-id()}">
		<xsl:apply-templates/>
	</a>
	</div>
...
<!--Supplies whitespace and punctuation if there is a pointer group with multiple entries-->
